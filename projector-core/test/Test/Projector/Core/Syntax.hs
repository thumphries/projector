...
prop_foldlExpr =
  gamble genExprIntAnn $ \expr ->
    foldlExpr (\acc e -> acc <> [extractAnnotation e]) [] expr
    ===
...
prop_foldlExprM =
  gamble genExprIntAnn $ \expr ->
    runState (foldlExprM (\acc e -> fun acc (extractAnnotation e)) 0 expr) []
    ===
...
prop_foldrExpr =
  gamble genExprIntAnn $ \expr ->
    foldrExpr (\e acc -> extractAnnotation e : acc) [] expr
    ===
...
prop_foldrExprM =
  gamble genExprIntAnn $ \expr ->
    runState (foldrExprM (\e acc -> fun (extractAnnotation e) acc) 0 expr) []
    ===
